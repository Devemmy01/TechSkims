...
const ProjectAnalytics = () => {
  ...
  const fetchAnalytics = async (range) => {
    try {
      ...
      // For "All Time", use the base endpoint without a range parameter
      const endpoint = range ? `${BASE_URL}/client/analytics/${range}` : `${BASE_URL}/client/analytics`;
      ...
      if (response.data.status === 'success') {
        ...
        // Safely extract values with fallbacks to 0
        ...
        // Ensure upcomingProjects is never negative
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center">
                    <span className="font-medium">{analytics.upcomingProjects}</span>
                    <span className="ml-2 text-xs px-2 py-1 bg-purple-100 text-purple-800 rounded-full">
                      {Math.min(calculatePercentage(analytics.upcomingProjects, analytics.totalProjects), 100)}%
                    </span>
                  </div>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${Math.min(calculatePercentage(analytics.upcomingProjects, analytics.totalProjects), 100)}%` }}
                  ></div>
                </div>
